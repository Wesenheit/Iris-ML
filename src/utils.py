...
from pystellibs import BaSeL, Bosz, BTSettl, Kurucz, Phoenix
...
M5 = {
    ...
    "II/319/las9": [
        ["Jmag1", "e_Jmag1", "Hmag", "e_Hmag", "Kmag", "e_Kmag"],
        ["2MASS_J", "2MASS_H", "2MASS_Ks"],
        0.5,
        [True, True, True],
    ],
...
class PhotometryGenerator(Star):
    ...
    def generate_data(self, T, g, Z, AV, RV, id):
        ...
        for j, i in enumerate(id):
            flux = self.fil_obj[i].get_flux(wave, spectrum) / (
                4 * np.pi * (10 * kpc) ** 2
            )
            try:
                flux = flux.value
    ...
    def log_prob(self, samples, id, mag, err):
        ...
        # different priors
        log_prob_AV = -self.p_core * (self.AV_core_mean - AV) ** 2 / (
            2 * self.AV_core_std**2
        ) - (1 - self.p_core) * (np.log(self.AV_exp) + self.AV_exp * (AV - 0.03))
        log_prob_RV = -((self.RV_mean - RV) ** 2) / (2 * self.RV_std**2)
        log_prob_MEH = -self.p_core * FEH**2 / (2 * 0.2**2) - (1 - self.p_core) * (
            self.p_pop2 * (-1.49 - FEH) ** 2 / (2 * 0.4**2)
            + (1 - self.p_pop2) * (-0.1 - FEH) ** 2 / (2 * 0.2**2)
        )
        log_prob_logg = 0
        ...
        dictionary["logT"] = np.log10(
            1000 * T
        )  # np.clip(np.log10(T*1000),self.lib_stell.log_T_min,self.lib_stell.log_T_max)
        ...
        for i in range(len(unique)):
            val = (
                self.fil_obj[unique[i]].get_flux(
        ...
        mag_broad = mag[:, np.newaxis, :].repeat(
            samples.shape[1], axis=1
        )  # mag with the shape boradcasted to the size of samples
        ...
        return_val = -np.sum(
            (sel_pred + factor - mag_broad) ** 2
            / (2 * err[:, np.newaxis, :].repeat(samples.shape[1], axis=1) ** 2),
            axis=-1,
        )
        # print(return_val[0,1:100])
        return_val = np.where(np.isnan(return_val), -np.inf, return_val) + (
            log_prob_AV + log_prob_RV + log_prob_MEH + log_prob_logg
        ).reshape(return_val.shape)
        return return_val
...
class BandDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        mags_torch, parameters_torch, what_torch = (
            torch.from_numpy(mags),
            torch.from_numpy(parameters),
            torch.from_numpy(what),
        )
        if self.noise:
            how_many = mags.flatten().shape[0]
            scale = (
                scipy.stats.truncnorm(loc=0.01, scale=0.04, a=0.000, b=0.2)
                .rvs(how_many)
                .reshape(mags.shape)
            )
            ...
            mags_torch = 22 - 2.5 * torch.log10(
                fluxes + torch.randn(fluxes.shape) * fluxes_err
            )
...
class DataBundle:
    def __init__(self, name, catalog=None, leng=0, bands=[0, 0, 0], B=128):
        ...
        dic["logg_best"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        )
        dic["[M/H]_best"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        )
        dic["AV_best"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        )
        dic["RV_best"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        ...
        dic["logg_16th"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        )
        dic["logg_50th"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        )
        dic["logg_84th"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        )
        dic["[M/H]_16th"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        )
        dic["[M/H]_50th"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        )
        dic["[M/H]_84th"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        )
        dic["AV_16th"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        )
        dic["AV_50th"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        )
        dic["AV_84th"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        )
        dic["RV_16th"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        )
        dic["RV_50th"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        )
        dic["RV_84th"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        ...
        dic["logg_true"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        )
        dic["logg_err"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        )
        dic["[M/H]_true"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        )
        dic["[M/H]_err"] = np.zeros(
            [
                leng,
            ],
            dtype=np.float64,
        ...
        if save_hp:
            for j in tqdm(range(len(self.file))):
                for i in range(self.data.shape[1]):
                    if (
                        self.data[j, i, 0] > 0 and self.data[j, i, 1] <= 0
                    ) or self.data[j, i, 1] > 0.1:
                        self.data[j, i, 0] = -1
    ...
    def get_photo(self, name, num, batch, **kwargs):
        idx = np.logical_and(
            np.arange(len(self.file)) >= num,
            np.arange(0, len(self.file)) < (num + batch),
        )
        if self.catalogs == None:
            print("No catalog found!")
            return
        if name not in self.unique_photo:
            self.unique_photo[name] = np.ones(
                [
                    len(self.file),
                ]
            ).astype(bool)
        if "ps1" in name:
            ...
            fil = {}
        v = Vizier(
            row_limit=-1, columns=columns, column_filters=fil, timeout=86400, **kwargs
        )
        count = 0
        while count < 10:
            try:
                result = v.query_region(
                    coords.SkyCoord(
                        ra=self.file["RA"].values[idx],
                        dec=self.file["DEC"].values[idx],
                        unit=(u.deg, u.deg),
                        frame="icrs",
                    ),
                    catalog=name,
                ...
                for i in range(len(self.catalogs[name][1])):
                    ...
                    #                        print("Something missing in {}!".format(name))
                        ...
                        err = (
                            0
                            if str(file[j][2 * i + 2]) == "--"
                            else float(file[j][2 * i + 2])
                        )
                        ampl = float(file[j][2 * i + 1])
                        name_test = (self.catalogs[name][1])[i]
                        name_new = (
                            self.con[name_test] if name_test in self.con else name_test
                        )
                        ...
                        if "ps1" in name:
                            ...
                            if check_bit(
                                file[j][11 + i], [2, 7, 5, 19, 47, 18, 40, 46, 44, 12]
                            ) and is_set(file[j][11 + i], 28):
                                add_flag = True
                            else:
                                add_flag = False
                        elif "smss" in name:
    ...
    def check_if_exist(self, name, num, batch, **kwargs):
        ...
        if name not in self.unique_remove:
            self.unique_remove[name] = np.ones(
                [
                    len(self.file),
                ]
            ).astype(bool)
    ...
    def upload(self, id, samples, best, log_prob):
        ...
        for i, num in enumerate(id):
            ...
            self.file.iloc[
                num, [offset, offset + 1, offset + 2, offset + 3, offset + 4]
            ] = best[i]
            for j in range(5):
                self.file.iloc[
                    num,
                ...
                ] = np.array([low[i, j], med[i, j], upp[i, j]])
