...
class TransformerBlock(nn.Module):
    def __init__(self,d_model:int,
        ...
        self.ff = nn.Sequential(nn.Linear(d_model,hidden),nn.GELU(),nn.Dropout(dropout),nn.Linear(hidden,d_model),nn.GELU())
...
class SEDTransformer(nn.Module):
    ...
    def __init__(self,bands:int,
        ...
        self.network = [ TransformerBlock(d_model=d_model,num_heads=heads,hidden=hidden,dropout=dropout) for _ in range(layers)]
...
class SED_NDE(nn.Module):
    def __init__(self,MAF_dict,Transformer_dict):
        ...
        self.preprocess = nn.Sequential(nn.Linear(Transformer_dict["d_model"],MAF_dict["cond_dim"]),
                                                  nn.GELU())
        self.embed_err = nn.Linear(1,Transformer_dict["d_model"],bias = False)

        self.loglike = lambda X: torch.where(X[:,:,-1] < 0,torch.zeros(X.shape[:-1]).to(X.device),torch.ones(X.shape[:-1]).to(X.device))

    def forward(self,target:torch.tensor,bands:torch.tensor,mags:torch.tensor,errors,dev = "cuda"):
    ...
    @torch.no_grad()
    def sample(self,how_many:int, bands:torch.tensor, mags:torch.tensor,errors = None,dev = "cuda",**kwargs):
        X_out = self.Transformer.pass_token(bands,mags,errors)
        X_out = self.preprocess(X_out)
    ...
    @torch.no_grad()
    def sample_SIR(self,how_many:int, bands:torch.tensor, mags:torch.tensor,errors = None):
        raise NotImplementedError
