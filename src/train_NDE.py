...
def train(args, log=True, save=5):
    ...
    data_set_test = BandDataset(
        os.path.join(data_dir, "test"), data_test["N"], noise=True
    )
    ...
    weights = torch.load(
        "SED_transformer_{}.tc".format(args.name_load), weights_only=True
    )
    ...
    if args.compile:
        ...
        train_model = model
    shed = torch.optim.lr_scheduler.CosineAnnealingLR(
        optim, args.num_epoche * len(data_loader)
    )
    loss_train_arr = []
    loss_test_arr = []
    if args.autocast:
    ...
    for n_ep in range(args.num_epoche):
        ...
        for X, errors, Y, bands in tqdm(data_loader, "train", disable=args.disable):
            ...
            for j in permutation:
                ...
                with torch.autocast(
                    device_type="cuda", dtype=torch.float16, enabled=args.autocast
                ):
        ...
        epoche_loss_test = 0
        train_model.eval()
        for X, errors, Y, bands in tqdm(
            data_loader_test, "evaluating", disable=args.disable
        ):
            X = X.to(device).float()
            Y = Y.to(device).float()
            bands = bands.to(device)
            ...
            for j in range(X.shape[0]):
                ...
                with torch.no_grad():
                    with torch.autocast(
                        ...
                        loss = torch.mean(
                            -train_model(Y_temp, bands_temp, X_temp, errors_temp)
                        )
                    epoche_loss_test += loss.item() / (
                        len(data_loader_test) * X.shape[0]
                    )
        if n_ep % save == 0 and n_ep > 0:
            MSE = np.zeros(5)
            for X, errors, Y, bands in tqdm(
                data_loader_test, "sampling", disable=args.disable
            ):
                X = X.to(device).float()
                Y = Y.to(device).float()
                bands = bands.to(device)
                ...
                for j in range(X.shape[0]):
                    ...
                    with torch.autocast(
                        ...
                        samples = (
                            model.sample(128, bands_temp, X_temp, errors_temp)
                            .cpu()
                            .numpy()
                        )
                    predicts = np.median(samples, axis=1)
                    MSE += (
                        np.mean((Y_temp.cpu().numpy() - predicts) ** 2, axis=0)
                        / X.shape[0]
                    )
        ...
        print(
            "Num epoche: {:.0f}, loss: {:.4f}, test loss: {:.4f}".format(
                n_ep, epoche_loss, epoche_loss_test
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=20,
        type=int,
        help="number of bands to load at once",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-na", "--name", required=True, type=str, help="name of the model"
    )
    parser.add_argument(
        "-a", "--autocast", type=str2bool, nargs="?", const=True, default=False
    )
    parser.add_argument(
        "-dir", "--dir", type=str, required=True, help="directory with the data"
    )
    parser.add_argument(
        "-dis", "--disable", type=int, default=True, help="disable tqmd"
    )
