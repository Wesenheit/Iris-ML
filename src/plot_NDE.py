...
def test_NDE(name, ra, dec, scale=1, eta=0.0, IS=False, MCMC=False, cuda=True):
    ...
    ).query_region(coord, radius=0.4 * u.arcsec, catalog="III/284/allstars")
    true = np.zeros(5) - 99
    try:
        catalogs = catalogs[0]
        print(catalogs)
    ...
    errors = torch.sqrt(
        torch.from_numpy(errors).float().to(dev).unsqueeze(1) ** 2 * scale**2 + eta**2
    )
    ...
    data_org = pd.DataFrame(
        columns=[r"$T$", "logg", "[M/H]", r"$A_V$", r"$R_V$"], data=samples_numpy
    )
    c = ChainConsumer()
    ...
    if IS:
        ...
        log_probs_pred = (
            ...
            * model(
                samples,
                bands.repeat_interleave(samples.shape[0], 1),
                mags.repeat_interleave(samples.shape[0], 1),
                errors.repeat_interleave(samples.shape[0], 1),
            )
            .cpu()
            .numpy()
        )
        ...
        for_IS[:, 0] /= 1000
        log_probs = generator.log_prob(
            for_IS.reshape(1, -1, 5), idx_IS, mags_IS, errors_IS
        ).squeeze(0)
        weights = scipy.special.softmax(log_probs - log_probs_pred)
        idx = np.random.choice(
            samples_numpy.shape[0], p=weights, size=samples_numpy.shape[0]
        )
        samples_SIR = samples_numpy[idx, :]
        data_SIR = pd.DataFrame(
            columns=[r"$T$", "logg", "[M/H]", r"$A_V$", r"$R_V$"], data=samples_SIR
        )
    ...
    if MCMC:
        ...
        sampler = emcee.EnsembleSampler(
            32,
            5,
            lambda x: generator.log_prob(
                x.reshape(1, -1, 5), idx_IS, mags_IS, errors_IS
            ).squeeze(0),
            vectorize=True,
        )
        ...
        data_MCMC = pd.DataFrame(
            columns=[r"$T$", "logg", "[M/H]", r"$A_V$", r"$R_V$"], data=samples_MCMC
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--low", type=float, default=0.01, help="lower bound of the error"
    )
