        ...
        for X,Y,bands in tqdm(data_loader_test,"evaluating",disable = args.disable):
            ...
            for j in range(X.shape[0]):
                ...
                with torch.no_grad():
                    ...
                    epoche_loss_test_T += loss_T.item()/(len(data_set_test)*data_test["B"])
                    epoche_loss_test_EBV += loss_EBV.item()/(len(data_set_test)*data_test["B"])
                    epoche_loss_test_metal += loss_metal.item()/(len(data_set_test)*data_test["B"])
        shed.step()
        print("Num epoche: {:.0f}, loss: {:.3f}, test loss temperature: {:.3f}, test loss EBV: {:.3f}, test loss metal: {:.3f}".format(i,
                                                                                                                               np.sqrt(epoche_loss),
...
if __name__=="__main__":
    ...
    parser.add_argument("-c","--cuda",type=str2bool, nargs='?',const=True, default = True)
    ...
    parser.add_argument("-com","--compile",type=str2bool, nargs='?', const=True, default = True)
